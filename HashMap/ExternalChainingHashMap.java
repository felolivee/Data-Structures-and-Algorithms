...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("You cannot add a null key or value");
        } else {
            if (((size + 1) / (double) table.length) > MAX_LOAD_FACTOR) {
                resizeBackingTable(table.length * 2 + 1);
            }
            int ind = Math.abs(key.hashCode() % table.length);
            ExternalChainingMapEntry<K, V> curr = table[ind];
            while (curr != null) {
            ...
            ExternalChainingMapEntry<K, V> head = new ExternalChainingMapEntry<>(key, value, table[ind]);
            table[ind] = head;
            size++;
            return null;
        }
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("You cannot remove a null key.");
        } else {
            int ind = Math.abs(key.hashCode() % table.length);
            ExternalChainingMapEntry<K, V> curr = table[ind];
            if (curr != null && key == curr.getKey()) {
                V val = curr.getValue();
                table[ind] = curr.getNext();
                size--;
                return val;
            }
            while (curr != null) {
                if (curr.getNext() != null && key == curr.getNext().getKey()) {
    ...
    public boolean containsKey(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Null keys do not exist.");
        } else {
            int ind = Math.abs(key.hashCode() % table.length);
            ExternalChainingMapEntry<K, V> curr = table[ind];
            while (curr != null) {
    ...
    public Set<K> keySet() {
        ...
        while (sizeTemp > 0) {
            if (table[currIndex] == null) {
                ...
                ExternalChainingMapEntry<K, V> curr = table[currIndex];
                while (curr != null) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("You cannot resize to smaller than the amount of entities.");
        } else {
            ...
            while (sizeTemp > 0) {
                if (table[currIndex] == null) {
                    ...
                    ExternalChainingMapEntry<K, V> curr = table[currIndex];
                    while (curr != null) {
                        int backingIndex = Math.abs(curr.getKey().hashCode() % temp.length);
                        ExternalChainingMapEntry<K, V> node = new
                                ExternalChainingMapEntry<>(curr.getKey(), curr.getValue());
                        node.setNext(temp[backingIndex]);
                        temp[backingIndex] = node;
                        curr = curr.getNext();
                        sizeTemp--;
                    }
                    currIndex++;
